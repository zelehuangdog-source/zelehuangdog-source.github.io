---
title: "最佳实践：从 Multi-Agent 到 Agent Team"
date: 2026-08-03
author: repost
categories: [转载, Agent架构与框架]
tags: [Agent架构, 多智能体协作, CodexLoom, Agent治理, 转载]
---

> **摘要**：本文以作者长期运行一支真实 Agent Team 的经验为线索，回答了一个核心问题——多个 Agent 并不会自动成为一支 Team。作者梳理了从 Task Agent 到 Long-running Agent、再到 Domain 分化的必然演化路径，并指出真正的瓶颈会从"单个 Agent 能力不足"转移到"Human 成为唯一的 Router"。破局的关键，是把原本只存在于人脑中的责任边界、协作关系、交接方式和当前状态外化为整个 Team 可查询、可验证的工作结构：用 Profile 定义 Identity/Domain/Scope，用 Agent Message 让 Agent 直接协作，用 Topic 为跨 Agent 工作保留唯一的当前版本和收口责任人，用 Overview 让 Team 的运行变得可观测、可治理，用受治理的 External 入口让内部能力安全地进入外部关系。文章反复强调各项能力的边界（Message delivered ≠ 工作正确，Topic resolved ≠ 现实结果完成，Signal ≠ Diagnosis，Membership ≠ 权限沙箱），并给出 Human 的新位置：不再是每一步的人工 Router，而是整支 Team 的 Owner。

> **转载声明**：本文**转载整理**自：《最佳实践：从Multi-Agent到Agent Team》，来源 https://mp.weixin.qq.com/s/ZloR4kbXacxpcEkIEv3oUQ。**内容版权归原作者及原出处所有**，本文仅供学习交流，**所有观点均属原作者，不代表本站立场**；如有侵权，请联系删除。
...
当 Agent 开始长期存在、形成不同 Domain，并且需要相互协作时，怎样把原本全部集中在 Human 脑子里的责任、关系、交接方式、当前状态和边界，逐渐变成整支 Team 可以使用的工作结构。
...
> 一条连续责任线依次穿过 Task、Long-running、Domain Agents、Human Router 和 Agent Team 五个阶段。机制示意：从一次 Task 到长期 Agent，再到 Domain 分化、Human Routing 和 Agent Team。Agent 数量增加只是中间阶段，真正的变化是协作责任开始从 Human 转移给 Agent。
...
> codexloom.ai 桌面首页，使用大字标题、暖白编辑画布和下方产品界面作为证据。CodexLoom 当前 Landing 页面。开头的外发故事来自这次真实上线，但页面上线、渠道发送和受众反应是三个不同的事实层级。
...
> 一条持续责任线穿过四个错落的 Task 切片，并把 Context、Correction 和 Boundary 带入 Long-running Agent。机制示意：Task 可以结束，责任仍会带着新的 Context、纠正和边界再次回来。Long-running 的意义，不是把一次对话无限拉长，而是让同一类责任持续回到同一个主体。
...
> 多种工作流进入一个不规则压力场，发生拥堵和冲突后从不同边界分化为多个 Domain Agents。机制示意：Scope 通常先扩张，直到不同 Context、方法和判断互相干扰，边界才从压力与能力劣化中显现。Domain 分化是对真实摩擦的回应。
...
> 多条并行 Agent lane 汇聚成 Human 前的 Context queue，再沿一条朱红色串行路由逐项输出。机制示意：多个 Domain Agent 可以同时工作，但如果 Context、选择和交接仍然必须经过同一个 Human，系统最终仍然从一个窄出口串行流出。
...
> CodexLoom Team Directory 展示多个匿名 Agent；Profile 详情分区显示 Identity、Domain 和 Scope。当前产品界面 · 隔离构造工作区。Team Directory 提供长期 Agent 的可查询入口；Profile 明确保存一个 Agent 当前声明的 Identity、Domain 与 Scope。声明是共同工作基线，不是能力、记忆或授权证明。
...
> CodexLoom Organization 关系图，一个父级 Agent 通过细线连接多个子级 Agent。当前产品界面 · 隔离构造工作区。Organization 可视化当前声明的 parent/child 长期责任结构；它不会自动选择 Owner、发送 Message 或授予权限。
...
> Collaboration 图显示多个匿名 Agent 之间的有向关系。当前产品界面 · 隔离构造工作区。Collaboration 保存有方向的长期协作接口；关系本身不自动完成交接、共享 Context 或授予权限。

> Activity 图显示一段时间内实际记录到的 Message 方向。当前产品界面 · 隔离构造工作区。Activity 按记录到的 Agent Message 呈现协作迹象。它可以帮助检验一条声明关系，但不是绩效，也不会自动把重复往来升级成 Collaboration。
...
> CodexLoom Agent Message 详情显示一条需要回复的请求，以及 reply、delivered、completed 和下方因果回复。当前产品界面 · 隔离构造工作区。Agent Message 分开记录请求的因果关系、送达状态与接收方处理状态；这些状态不等于业务结果正确。
...
> 左右两个独立 Context 之间，一条折返的因果消息路径经历 Open、Correct、Align 和 Converge。机制示意：两个 Agent 保留各自长期 Context，通过 Open、Correct、Align 和 Converge 逐轮增加共同理解。多轮不是把一条完整消息拆碎，而是让真实返回成为下一轮的新 Context。
...
> 三个独立 Agent Thread 通过 Message 围绕中央 Topic 协作，Topic 分层显示 Responsible、Current、Waiting、Evidence 和 Result。机制示意：Message 连接局部 request 与 reply；Topic 由唯一 Responsible 维护 Current、Waiting、Evidence 和 Result。参与者仍在各自 Thread 中工作。
...
> CodexLoom Topic Current 页面显示 brief 版本、等待对象、下一步、限制、Responsible、Participants、Purpose 与完成条件。当前产品界面 · 隔离构造工作区。Topic Current 由唯一 Responsible 维护共享 brief、等待条件、下一步、限制和 scoped Participants；各 Agent 的高分辨率工作仍留在自己的 Thread。
...
> Needs You 页面显示一个有背景说明、两个选项和回答输入区的 Human 决策请求。当前产品界面 · 隔离构造工作区。Needs You 把需要 Human 提供事实、选择、Review 或授权的精确阻断点持久化。问题被回答，不等于后续动作已经发生或有效。
...
> Overview Status 显示四个摘要、日期、八名 Agent 的 30 分钟活动时间轴、图例、Tracked 8/8 和 Asia/Shanghai。当前产品界面 · 隔离构造工作区。Status 和 Daily Activity 压缩当前状态与记录到的执行节奏；Tracked、时区和图例限定读数范围。这些是调查信号，不是绩效评分。
...
> Overview Capacity 页面显示五个 signal cards、时间图、来源分布和 Agent table；Data quality 说明 Capacity 统计口径与已知限制。当前产品界面 · 隔离构造工作区。Capacity 将 recorded execution、new-work wait 与 current backlog 放在同一观察面，帮助 Owner 选择下一步调查对象；单个页面不能证明瓶颈、历史趋势、利用率或原因。
...
> Overview Token Usage 页面显示总量、每日堆叠图、图例和各 Agent 的 allocation。当前产品界面 · 隔离构造工作区。Token Usage 展示选定时段内不同 token 类型、模型调用和 Agent allocation 的分布；它不等于成本效率、质量、重要性或业务价值。
...
> Signal 进入多条 Evidence 记录，再汇聚到 Human Diagnosis、Small Change，并沿 Next Work 回到新的观察。机制示意：运行信号先缩小调查范围，再沿证据下钻，由 Human 判断原因、尝试小幅可逆调整，最后用下一轮真实工作验证。Signal 不是 Diagnosis。
...
> 左侧 Internal correction loop 与右侧多种不确定 External input 被一条垂直 Trust Boundary 分隔，输入从 Governed Entry 进入。机制示意：内部协作建立在长期纠正与熟悉边界上；外部输入可能带有错误前提、越权意图或对抗性。受管入口改变通信路径，但不是完整安全隔离。
...
> CodexLoom External 页面显示已连接的飞书身份、启用的匿名 Membership，以及 When mentioned、Send final answer、Proactive + replies 策略。当前产品界面 · 匿名真实运行记录。一个已连接的飞书身份及其已启用 Conversation Membership。Membership 保存该 Conversation 的局部角色与 mention、reply、outbound 策略；名称与标识已匿名。
...
> Identity、Membership、Interface、Internal Team、Human、Outbox 和 Delivery 以分层协议结构连接。机制示意：External 不是接上一个 Bot。Identity、Conversation Membership、Interface、可选的 Internal Team 协作、Human 边界、Outbox 与 Delivery 分别保存不同责任。
...
> CodexLoom Inbox 显示一条匿名飞书消息为 handled，处理尝试为 1，使用 Membership v2，并显示 Reply sent；Outbox 显示一条匿名飞书回复为 sent，尝试次数为 1，并保留已脱敏的 provider message result。当前产品界面 · 匿名真实运行记录。它不证明对方已读、理解、接受或采取行动。
...
当成熟的 Domain 能力可以通过这样的 Interface 进入外部，同时保留可检查的外部身份、Conversation policy、Human 决策点和结果回流记录，Agent Team 才开始从一套内部生产力系统，变成一种能够持续对外交付的组织能力。
...
> 五条稳定 Agent thread 进入不规则 Dynamic Team surface，Human Owner 在一侧 Observe 与 Adjust，结果沿 Validate records 回到下一轮。机制示意：稳定的是长期责任主体，动态的是当前组织假设。Human Owner 通过观察、调整和后续真实工作验证，让 Team 随模型、工具、业务和外部环境持续演化。
